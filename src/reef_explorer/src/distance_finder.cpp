...
DistanceFinder::DistanceFinder(const std::string& rangeImageTopic, const std::string& jointTopic, const std::string& panTopic, const std::string& tiltTopic, double splitFactorHeight, double splitFactorWidth, volatile int* syncState) : it_(nh_)
{
    ...
    this->cloud_sub = this->nh_.subscribe("/octomap_point_cloud_centers", 1, &DistanceFinder::cloudCb, this);
    this->state = 0;
    this->moveSensor = false;
    this->movingSensorAtm = false;
...
{
    if(*syncState != 0 && this->movingSensorAtm == false)
    {
        this->movingSensorAtm = true;
        this->moveSensor = false;
        float sleepFreq = 480;
...
void DistanceFinder::cloudCb(const sensor_msgs::PointCloud2ConstPtr& input)
{
    this->moveSensor = true;
}
